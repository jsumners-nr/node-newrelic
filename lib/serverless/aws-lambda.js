...
class AwsLambda {
  ...
  isResponseStreamingLambda(handler) {
    const HANDLER_STREAMING = Symbol.for('aws.lambda.runtime.handler.streaming')
    const isStreaming = Object.prototype.hasOwnProperty.call(handler, HANDLER_STREAMING)
    if (isStreaming) {
      const metric = this.agent.metrics.getOrCreateMetric(
        NAMES.SUPPORTABILITY.API + '/isResponseStreamingLambda'
      )
  ...
  patchLambdaHandler(handler) {
    ...
    // determine if this is a streamified handler or not
    const isStreaming = this.isResponseStreamingLambda(handler)

    if (isStreaming) {
      // if it *is* streaming, add a transaction ender
      transactionEnders.push((stream) => new Promise((resolve) => {
        if (stream.writableFinished) {
          resolve(stream.writableFinished)
        }
      }))
    }
    ...
    function wrappedHandler() {
      ...
      const event = args[0]
      const context = isStreaming ? args[2] : args[1]
      ...
      if (isStreaming) {
        // the last arg in response streaming won't be a callback
        // the response stream is the second arg; need to listen for the stream end
  ...
  wrapStreamAndCaptureError(transaction, txnEnder, stream, resultProcessor) {
    const shim = this.shim
    stream.on('error', err => {
      shim.agent.errors.add(transaction, err)
    })
